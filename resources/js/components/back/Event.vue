					        ...
					        <div class="modal-dialog">
					          <div class="modal-content bg-success">
					            ...
					            <div class="modal-body">
					                ...
					                <div class="form-group mb-1">
					                    <label>Rating</label>
					                    <input v-model="form.rating" type="text" name="rating"
					                      class="form-control" :class="{ 'is-invalid': form.errors.has('rating') }" placeholder="Rating">
                              ...
                              <table class="table table-hover">
                                ...
                                <tbody>
                                  ...
                                  <tr v-for="(result, index) in customers" :key="result.id">
                                    ...
                                    <td>{{ result.rating }}</td>
                                    <td>
                                        <img :src="'/uploads/event/'+result.image" alt="No image" style="height: 60px; width: 60px;border-radius:50%;">
                                    </td>
...
<script>
    export default {
    	...
    	methods: {
    		...
    		getData(){
                axios.get('/api/event?page='+this.pagination.current_page)
                .then(response => {
            ...
            searchData(){
                axios.get('/api/search/event/'+this.queryField+'/'+this.query+'?page='+this.pagination.current_page)
           ...
           onChange(e){
    		...
    		createUser(){
    			this.form.busy = true;
        		this.form.post('/api/event/',{image: this.avatar})
        		.then(response => {
        			this.getData()
        			$('#addModal').modal('hide');
    		...
    		edit(result){
                ...
                this.avatar = "/uploads/event/" + this.form.image;
        		$('#addModal').modal('show');
            ...
            unactive(id){
        		...
        		axios.get('/api/event/unactive/'+id)
                .then(response => {
                    this.getData()
    		...
    		destory_info(){
    			this.form.delete('/api/event/'+this.form.id)
